    ...
    <SafeAreaView edges={['top', 'bottom']} className="flex-1">
      <>
        <View className="items-center justify-center gap-2 px-6 pt-10">
          <ThemedText>{t('home.remainingBudget')}</ThemedText>
          <ThemedText
            variant="primary"
            className={cn(
          ...
          <ConditionalWrapper conditional={userHasTransactions}>
            <View className="flex-row gap-3">
              <Badge
                label={t('home.income', {
                  amount: `${currency} ${transformValueToCurrency(budget?.toString() ?? '0', true)}`,
                })}
              ...
              <Badge
                label={t('home.spent', {
                  amount: `${currency} ${transformValueToCurrency(totalExpenses.toString(), true)}`,
                })}
