...
function RadioGroup({
  ...
  return <RadioGroupPrimitive.Root className={cn('w-full gap-3', className)} {...props} />;
}
...
function RadioGroupItem({
  ...
  return (
    <RadioGroupPrimitive.Item
      className={cn(
        'border-gray-gray500 aspect-square size-4 shrink-0 items-center justify-center rounded-full border bg-transparent shadow-sm shadow-black/5',
        Platform.select({
      ...
      {...props}
    >
      <RadioGroupPrimitive.Indicator className="bg-brand-brand600 size-2 rounded-full" />
    </RadioGroupPrimitive.Item>
  );
}
